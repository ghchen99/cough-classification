...
public class MainActivity extends Activity implements MainContract.View, View.OnClickListener, SensorEventListener {
...
//							}
	...
	public void updateOrientationAngles() {
		...
		String location = "";
		if ((Math.abs(orientationAngles[1]) < 0.1) & (Math.abs(orientationAngles[2]) < 0.5)) {
			location = "On table";
		}
		else if ((Math.abs(orientationAngles[1]) > 0.3) | (Math.abs(orientationAngles[2]) > 0.5))  {
			location = "In pocket";
		}
		Log.d(TAG, "onSensorChanged: Pitch: " + orientationAngles[1] + " Roll: " + orientationAngles[2] + " Azimuth: " + orientationAngles[0]);
		Log.d(TAG, "onSensorChanged: Phone Location: " + location);
	}
